...
- **קליטת לידים (Lead Ads)** — webhook מאומת-HMAC שקולט לידים מטופסי Meta בזמן אמת לתיבת לידים עם ניהול סטטוס.
...
- **בדיקת מובהקות A/B** — מבחן z לשתי פרופורציות על יחס ההמרה בין וריאציות הקמפיין (מנצח, p-value, ביטחון, שיפור יחסי) והמלצה פעולה — מנוע דטרמיניסטי בדוק ב-marketing-core.
...
- טוקנים מוצפנים ב-AES-256-GCM (לעולם לא בטקסט גלוי).
